...
export default async function WebhookPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  if (membership?.role !== "owner") redirect(`/campaigns/${slug}`);
  ...
  return (
    <div style={{ padding: "28px 36px" }}>
      <nav style={{ fontSize: 13, marginBottom: 24, display: "flex", gap: 6, alignItems: "center" }}>
        ...
        <span style={{ color: "var(--text-muted)" }}>/</span>
        <span style={{ color: "var(--text-primary)", fontWeight: 500 }}>Webhook</span>
      </nav>
      <WebhookConfigClient campaignSlug={slug} orgId={session.orgId} />
    </div>
  );
}
