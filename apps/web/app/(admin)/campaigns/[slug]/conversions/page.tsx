...
export default async function ConversionsPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  if (!membership || membership.role === "viewer") redirect(`/campaigns/${slug}`);
  ...
  return (
    <div style={{ padding: "28px 36px"}}>
      <nav style={{ fontSize: 13, marginBottom: 24, display: "flex", gap: 6, alignItems: "center" }}>
        ...
        <span style={{ color: "var(--text-muted)" }}>/</span>
        <span style={{ color: "var(--text-primary)", fontWeight: 500 }}>Conversions</span>
      </nav>
      <ConversionsDashboard campaignSlug={slug} />
    </div>
  );
}
